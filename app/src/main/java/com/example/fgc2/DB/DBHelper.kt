...
internal class MyDatabaseHelper(private val context: Context?) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val query =
            "CREATE TABLE $TABLE_NAME ($COLUMN_ID INTEGER PRIMARY KEY AUTOINCREMENT, $COLUMN_TITLE TEXT, $COLUMN_DESCRIBE TEXT);"
        db.execSQL(query)
    }
    ...
    fun addSets(title: String?, author: String?) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put(COLUMN_TITLE, title)
        cv.put(COLUMN_DESCRIBE, author)
    ...
    companion object {
        private const val DATABASE_NAME = "SetsLibrary.db"
        private const val DATABASE_VERSION = 1
        private const val TABLE_NAME = "my_library"
        private const val COLUMN_ID = "_id"
        private const val COLUMN_TITLE = "sets_title"
        private const val COLUMN_DESCRIBE = "sets_describe"
    }
}